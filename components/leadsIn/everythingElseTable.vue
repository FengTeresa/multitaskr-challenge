...
export default {
  ...
  created() {
    this.handleGetEverythingEData()
  },
  mounted() {
    this.totalRows = this.dataSource.length
  },
  methods: {
    handleGetEverythingEData() {
      for (const value of Object.values(this.dataSource)) {
        const date = moment(value.dateTime.date, 'DD/MM/YYYY')
        const todaydate = moment().format()
        const days = date.diff(todaydate, 'days') * -1

        if (days >= 2) {
          this.everythingEData.push(value)
        }
      }
    },
  },
